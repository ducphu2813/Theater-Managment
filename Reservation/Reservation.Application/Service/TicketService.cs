...
using Reservation.Application.Helper;
...
public class TicketService : ITicketService
{
    ...
    public async Task<Dictionary<string, object>> GetAllAsync(int page, int limit)
    {
        ...
        //lấy tất cả vé
        var tickets = await _ticketRepository.GetAll(page, limit);
        
        //duyệt qua từng vé để chuyển đổi múi giờ
        foreach (var ticket in tickets["records"] as List<Ticket>)
        {
    ...
    public async Task<Dictionary<String, Object>> GetByIdAsync(string id)
    {
        ...
        if (ticket == null)
        {
            throw new MongoException($"Ticket with id {id} was not found.");
        }
    ...
    public async Task<Ticket> AddAsync(SaveTicketDTO ticket)
    {
        ...
        if(seats.Count == 0)
        {
            throw new MongoException("Seat not found");
        }
